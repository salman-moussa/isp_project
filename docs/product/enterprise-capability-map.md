...
| Warehouse and procurement            | Items/SKUs, serialized assets, vendors, quotes, POs, receiving, warehouses/bins, transfers, reservations, issue/return, RMA, reorder, valuation                                       | `partial`                         | Serialized issue/install/return/RMA is scoped, versioned and evidenced; administration, procurement, reservation and valuation remain. |
...
Continue from the locally proven serialized custody vertical into governed procurement, receiving,
reservation and valuation, then an explicit NAS/RADIUS activation boundary. Inventory intake and
financial valuation must remain atomic with accounting evidence; network actions retain exact
idempotency, canonical scope, action-specific acknowledgement and fail-closed provider behavior.
Telemetry, alarm correlation, SLA and communications remain the next NOC expansion. Relocation
remains coupled to the field/resource delivery tranche.
