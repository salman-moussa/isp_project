...
AR/AP/treasury remain unfinished. Warehouse, telemetry, RADIUS execution and mobile hardware
...
Continue from the proven financial-source and manual NOC incident verticals into serialized
warehouse custody and an explicit NAS/RADIUS activation boundary. Inventory issue/return must be
atomic with field work and accounting evidence; network actions must retain exact idempotency,
canonical scope, action-specific acknowledgement and fail-closed provider behavior. Telemetry, alarm
