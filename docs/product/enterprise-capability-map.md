...
| Warehouse and procurement            | Items/SKUs, serialized assets, vendors, quotes, POs, receiving, warehouses/bins, transfers, reservations, issue/return, RMA, reorder, valuation                                       | `partial`                         | Custody, vendors, valued POs, MFA approval, serialized receipt, Inventory/AP posting, and scoped catalog/warehouse/bin administration are live; quote comparison, partial and non-serialized receipts, reservations, bin-level stock and transfers remain. |
...
Catalog, warehouse and bin administration is now delivered with local acceptance evidence
(2026-09-05): an ISP can create and version its own SKUs, warehouses and bins instead of relying on
a DBA insert. Continue into partial and non-serialized receipts, bin-level stock, reservations and
transfers, then an explicit NAS/RADIUS activation boundary. Inventory intake and financial valuation
must remain atomic with accounting evidence; network actions retain exact idempotency, canonical
scope, action-specific acknowledgement and fail-closed provider behavior. Telemetry, alarm
correlation, SLA and communications remain the next NOC expansion. Relocation remains coupled to the
field/resource delivery tranche.
