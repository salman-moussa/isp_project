...
valued adjustments are delivered with local acceptance evidence (2026-09-05). Continue into stock
counts, RMA/repair and reorder suggestions, then an explicit NAS/RADIUS activation boundary.
Inventory intake and financial valuation must remain atomic with accounting evidence; network
actions retain exact idempotency, canonical scope, action-specific acknowledgement and fail-closed
provider behavior. Telemetry, alarm correlation, SLA and communications remain the next NOC
expansion. Relocation remains coupled to the field/resource delivery tranche.
