; control-to-tenant membership mirror on the two-database layout# Orvex ISP enterprise release
status

Status: live engineering ledger Controlling specification:
[`product/enterprise-capability-map.md`](product/enterprise-capability-map.md) Updated: 2026-09-14
...
control database (sign-in, invitations, role changes), while operational tables in the tenant
database reference `tenant_memberships(tenant_id, user_id)` for assignees, collectors, approvers and
authors. On the two-database production layout nothing kept the tenant side current after bootstrap,
so staff invited later could sign in but could not own or be assigned operational records.

Migration 202609140100_tenant_directory_mirror.sql (tenant) adds `mirror_tenant_directory(jsonb)`: a
SECURITY DEFINER function that applies one tenant's directory snapshot (tenant row, member
identities, memberships) under that tenant's own context. It inserts a placeholder password hash
only on insert and never changes an existing hash (the tenant database authenticates nobody), so on
a single-database deployment, where the tables are the same, a pass is a no-op; it writes updates
only when a value differs, so the authorization-version trigger does not fire and no session is
ended by a mirror pass; memberships the control database no longer lists are deactivated, never
deleted, because operational rows keep referencing them. The mirror copies permissions verbatim and
grants nothing: sign-in still derives authority from the control database.

API: `TenantDirectoryMirror` (source: control-database read of the tenant directory; target: the
tenant database function) runs after every accepted invitation and every membership update in
`TenantStaffService`, and once at API start-up for every tenant with per-tenant failures logged
without aborting the others (no secrets are logged). Contracts gain `TenantDirectorySnapshot`.

Suites: staff service unit test (mirror called after accept and after a role change); live
PostgreSQL 18 (`packages/database/scripts/test-live-directory-mirror.ts`): control-side snapshot
read with scope, unknown tenant yields null, same-database pass changes nothing and keeps the real
password hash and authorization version, a snapshot for an unknown tenant creates the tenant,
identity (placeholder hash, MFA flag) and membership that an operational row can reference, a role
change updates, a removal deactivates, and a malformed payload is refused. Runs in the integration
chain after the preset check.
...
(`tenant_role_permissions`) still lacked the three accounting permissions that the contracts presets
carry, so an owner, administrator or finance member created or re-assigned through the product could
not open the accounting workspace. The production bootstrap membership also kept the non-canonical
role key `administrator` from 2026-08-14, which the canonical re-synchronisation in the
sales-permissions migration skipped, leaving it at 23 of 30 permissions.

Migration 202609110100_control_accounting_permissions.sql (control) replaces the preset function to
mirror `tenantRolePresets` exactly, renames legacy `administrator` memberships to
`isp_administrator`, re-synchronises every membership with its preset, bumps the authorization
version and ends the affected tenant sessions with reason `canonical_permissions_upgraded`; MFA
flags are not touched by the migration. Migration 202609110200_tenant_membership_mirror.sql (tenant)
aligns the mirrored owner and administrator rows in the tenant database so directory reads
(assignees, team members, approvers) match what sign-in grants. New live check
`packages/database/scripts/test-live-role-presets.ts` compares the database preset of every
...
reads that join `tenant_memberships` will not list them until a mirror exists. Organization and IAM
stays `partial` with this item on its remaining list.
...
Counting the map's state column: 29 `partial`, 3 `foundation`, 1 `verified`, 1
`activation_required`, 0 `missing`. Capacity and upstream management, the last `missing` row, moved
to `partial` on 2026-09-11 with the section below. `partial` means the vertical runs in production
with live proof but the map lists further depth; nothing in the tenant shell, the Control Center,
the field manual or LearnISP shows demonstration content.

Evidence for this checkpoint, all on 2026-09-11 against the working tree that becomes the next
deploy:

| Gate                                                              | Result                                                                                                                                                                                                                                                                                                                                |
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `typecheck`, `lint`, `format:check`                               | pass                                                                                                                                                                                                                                                                                                                                  |
| `npm test` (API, tenant-web, platform-web, ui, workers, packages) | pass                                                                                                                                                                                                                                                                                                                                  |
| `db:check`, `brand:check`, `learnisp:check`                       | pass                                                                                                                                                                                                                                                                                                                                  |
| Fresh PostgreSQL 18 live chain                                    | 21 scripts pass in CI order on an empty database: bootstrap, finance, tenant staff, sales, customer accounts, financial journals, NOC, inventory, integrations, field service, network resources, NOC telemetry, dealers, assurance, customer service, analytics, cashier/collections, control portfolio, regulatory/people, capacity |
| Production checkpoints                                            | `93e0d4e`, `ec018d5`, `a2a047f`, `6ec25c5`, `737df65` deployed with `Deployment complete.`, backups verified, invariants clean                                                                                                                                                                                                        |

Rules re-checked on this pass: tenant context only from the signed session or a scoped support
grant; deny by default with the canonical permission catalogue (no new permission names were
introduced; capacity reuses `tenant.network.job.create`, `tenant.network.view` and
`tenant.report.view`); platform staff hold no implicit tenant access; money as integer minor units
with ISO currency, USD and LBP never combined; posted financial records append-only with linked
reversals; exact idempotency on every mutation; subscriber service never suspended because of
platform subscription state; secrets as secret-manager references, never logged or committed; every
user-facing string in English and Arabic; no subscriber portal or login; applied migrations
unchanged (the deploy preflight checksums them).

Items that stay `activation_required` or need a business decision, contract, credential or hardware
rather than code: platform SMTP and SMS provider credentials; payment provider contracts and
settlement files (cards, bank, OMT, Whish, LibanPost, Cash United); Expo/EAS project, Android
signing keys, Play Console listing and an approved Bluetooth receipt printer for the Collect app;
regulator portals and file formats; a reporting job in every deployment so the Control Center
deployments page stops reading "not reporting"; router counter feeds for automatic capacity samples.

## Production checkpoint deployed — 2026-09-10 (`737df65`, LearnISP and regulatory/people)

Release id `20260910T072944Z-737df65`; the deploy script completed end to end with
`Deployment complete.`

| Item                | Result                                                                                                                                    |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| Backup              | `/opt/orvex-backups/20260910T072944Z-737df65`, verified with SHA256SUMS                                                                   |
| Migrations promoted | 1 (`202609100100_tenant_regulatory_people`); earlier applied files preserved at their applied bytes                                       |
| Endpoints           | `/ready` 200, `/` 200, `/control/` 200, `/guide.html` 200, `/learnisp/` 200, `/learnisp/payments/` 200, `/learnisp/search-index.json` 200 |
| Invariants          | unbalanced journals 0, invalid indexes 0                                                                                                  |

What is now live: the Regulatory & QoS and People & shifts workspaces, the regenerated field manual
and the LearnISP reference with search and one route per module.

Rollback boundary: `/opt/orvex-backups/20260910T072944Z-737df65/source.tar` plus both database dumps
and `env.backup`.

## Capacity and upstream management — 2026-09-11

The last `missing` domain. Migration 202609100200_tenant_capacity_upstream.sql adds it under the
signed operations context: `execute_capacity_command` (action `tenant.capacity.manage`, permission
`tenant.network.job.create`) and `read_capacity_workspace` (action `tenant.capacity.workspace.read`;
`tenant.network.view`, `tenant.network.job.create` or `tenant.report.view`).

Register (`operations_upstream_circuits`): code unique per tenant, provider, kind (transit, peering,
Ogero DSL, Ogero fibre, wireless backhaul, leased line, other), point of presence, optional branch
(checked against the caller's scope), committed and burst Mbps (burst never below committed),
monthly cost as integer minor units with its ISO currency, contract start and end, renewal notice
days, SLA availability, status (planned, active, decommissioned), notes and an optimistic version.
Samples (`operations_capacity_samples`): peak and average inbound/outbound Mbps at a timestamp,
unique per circuit and timestamp so re-imports never double count, append-only by trigger; a
decommissioned circuit takes no samples. Every command is recorded in `operations_capacity_events`
(append-only, unique idempotency key, exact replay returns the stored result with `replayed: true`,
a reused key with different content is refused) and in the operations audit outbox with before and
after values.

Workspace: for a window of 7–365 days each circuit carries its latest sample, window peak,
utilisation and headroom against committed capacity, a rating (`ok` below 75%, `warning` from 75%,
`critical` from 90%, `unknown` without samples, `none` when not active), growth per month from a
linear regression of peaks over time, months to saturation when growth is positive, a trend series,
days to contract end and `renewalDue` inside the notice period; circuits sort by risk. Totals give
committed Mbps and counts per status; cost is summed per currency for active circuits and never
combined across currencies.

Tenant web: "Capacity & upstream" workspace (EN/AR, RTL) with four figures (committed capacity,
window peak, saturation risk with watch count, renewals due with cost per currency), a circuit table
with utilisation meter, sparkline against the committed line, rating, forecast and renewal pill, a
details panel per circuit with its samples, and forms to add or edit a circuit and to record a
sample; the money field converts USD to cents and keeps LBP whole. API routes
`POST /v1/tenants/:tenantId/operations/capacity/commands` and
`GET /v1/tenants/:tenantId/operations/capacity/workspace?days=`.

Suites: API (authority binding, refused unqualified timestamp, viewer cannot write), tenant-web
(figures and ratings, Arabic sample recording, circuit creation with minor-unit conversion), live
PostgreSQL 18 (`packages/database/scripts/test-live-capacity.ts`: read permission cannot write,
exact replay and refused reuse, unique code, LBP and USD cost rows kept apart, ten rising samples
giving 92.5% utilisation, critical rating, positive growth and sub-month saturation forecast,
duplicate timestamps ignored, append-only samples, stale version refused, burst below committed
refused, decommissioned circuits refuse samples and carry no risk, branch outside scope refused,
audit and event rows recorded). Not yet: router counter ingestion, POP/sector/OLT saturation,
provider upgrade orders and SLA credit claims.
...
| Capacity and upstream management     | `partial`                         | Upstream circuit register (transit, peering, Ogero DSL/fibre, wireless backhaul, leased line) with provider, POP, branch, committed and burst capacity, monthly cost per currency, contract dates, renewal notice and SLA; append-only utilisation samples; workspace deriving peak, utilisation, headroom, saturation rating, growth, months to saturation, trend and renewals due | `tenant.network.job.create` to manage; `tenant.network.view`, `tenant.network.job.create` or `tenant.report.view` to read; signed context, FORCE RLS, branch scope on the branch link                      | Operations audit on circuits and sample batches; append-only capacity event ledger with exact replay                                    | Live PostgreSQL 18 register/sample/forecast/version/scope proof (test-live-capacity.ts); API and UI suites                                            | Router counter ingestion, POP/sector/OLT saturation, provider upgrade orders, SLA credit claims                                      |
