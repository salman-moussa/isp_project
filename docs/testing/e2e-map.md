...
| E2E-T02  | Recurring billing and retry failed subscribers                       | Tenant/API/jobs             | Durable per-service outcomes, no duplicates, currency separation and failed-only retry |
...
| E2E-T02B | Discount, VAT, stamp and bilingual legal first invoice               | Tenant/API/web/database     | Versioned policy, exact components, legal identity snapshot and currency separation    |
| E2E-T02C | Governed overdue evaluation and dunning review                       | Tenant/API/web/database     | Versioned thresholds, bilingual evidence, exact replay and no automatic network action |
