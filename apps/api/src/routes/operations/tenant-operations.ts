...
export function registerTenantOperationsRoutes(
  ...
  const routes: readonly RouteSpec[] = [
    ...
    operation(
      ...
      'support_issue',
      z.object({ command: supportCommandSchema }).strict(),
      (w, id, v) => w.executeSupportCommand(id, v as never),
    ),
    operation(
      ...
      'message_template',
      z.object({ command: templateCommandSchema }).strict(),
      (w, id, v) => w.executeTemplateCommand(id, v as never),
    ),
    operation(
      ...
      'tenant.communication.manage',
      'notification',
      z.object({ command: communicationCommandSchema }).strict(),
      (w, id, v) => w.executeCommunicationCommand(id, v as never),
    ),
    operation(
      ...
      z.object({ command: deliverNotificationsSchema }).strict(),
      (w, id, v) => w.deliverNotifications(id, v as never),
    ),
    operation(
      '/assurance/commands',
    ...
    operation(
      ...
      'regulatory',
      z.object({ command: regulatoryCommandSchema }).strict(),
      (w, id, v) => w.executeRegulatoryCommand(id, v as never),
    ),
    operation(
      '/people/commands',
      ...
      'people',
      z.object({ command: peopleCommandSchema }).strict(),
    ...
    operation(
      ...
      'cashier',
      z.object({ command: cashierCommandSchema }).strict(),
      (w, id, v) => w.executeCashierCommand(id, v as never),
    ),
    operation(
      '/cashier/void',
      'voidReceipt',
      ...
      'tenant.cashier.manage',
      'cashier_void',
      z.object({ command: voidReceiptSchema }).strict(),
      (w, id, v) => w.voidReceipt(id, v as never),
      true,
    ),
    operation(
      ...
      'collections',
      z.object({ command: collectionManageCommandSchema }).strict(),
      (w, id, v) => w.executeCollectionCommand(id, v as never),
    ),
    operation(
      '/collections/approve',
      'approveRouteSettlement',
      'tenant.collection.reconcile',
      'tenant.collection.manage',
      'collections_settlement',
      z.object({ command: approveSettlementSchema }).strict(),
      (w, id, v) => w.executeCollectionCommand(id, v as never),
      true,
    ),
    operation(
      ...
      z.object({ command: recordCollectionSchema }).strict(),
      (w, id, v) => w.recordCollection(id, v as never),
    ),
    operation(
      '/dealers/channel',
  ...
  registerWorkspaceRead(
    ...
    options,
    {
      path: '/v1/tenants/:tenantId/operations/support/workspace',
      operationId: 'readSupportWorkspace',
      permission: 'tenant.subscriber.view',
      action: 'tenant.support.workspace.read',
      resourceType: 'support_workspace',
      schema: z.object({}).strict(),
      querySchema: supportQuerySchema,
      execute: (w, id, v) => w.readSupportWorkspace(id, v as never),
    },
    'Tenant customer service',
    'support-read',
    'Read the scoped customer service ticket workspace',
  );
  registerWorkspaceRead(
    app,
    options,
    {
      path: '/v1/tenants/:tenantId/operations/communications/workspace',
      operationId: 'readCommunicationsWorkspace',
      permission: 'tenant.subscriber.view',
      action: 'tenant.communication.workspace.read',
      resourceType: 'communications_workspace',
      schema: z.object({}).strict(),
      execute: (w, id, v) => w.readCommunicationsWorkspace(id, v as never),
    },
    'Tenant communications',
    'communications-read',
    'Read templates, consents and the notification outbox',
  );
  registerWorkspaceRead(
    app,
    options,
  ...
  registerWorkspaceRead(
    ...
    options,
    {
      path: '/v1/tenants/:tenantId/operations/regulatory/workspace',
      operationId: 'readRegulatoryWorkspace',
      permission: 'tenant.report.view',
      action: 'tenant.regulatory.workspace.read',
      resourceType: 'regulatory_workspace',
      schema: z.object({}).strict(),
      querySchema: regulatoryQuerySchema,
      execute: (w, id, v) => w.readRegulatoryWorkspace(id, v as never),
    },
    'Tenant regulatory',
    'regulatory-read',
    'Read licences, obligations, KPI evidence and submissions',
  );
  registerWorkspaceRead(
    app,
    options,
    {
      path: '/v1/tenants/:tenantId/operations/people/workspace',
      operationId: 'readPeopleWorkspace',
      permission: 'tenant.user.administer',
      action: 'tenant.people.workspace.read',
      resourceType: 'people_workspace',
      schema: z.object({}).strict(),
      querySchema: peopleQuerySchema,
      execute: (w, id, v) => w.readPeopleWorkspace(id, v as never),
    },
    'Tenant people',
    'people-read',
  ...
  registerWorkspaceRead(
    ...
    options,
    {
      path: '/v1/tenants/:tenantId/operations/cashier/workspace',
      operationId: 'readCashierWorkspace',
      permission: 'tenant.payment.view',
      action: 'tenant.cashier.workspace.read',
      resourceType: 'cashier_workspace',
      schema: z.object({}).strict(),
      querySchema: cashierQuerySchema,
      execute: (w, id, v) => w.readCashierWorkspace(id, v as never),
    },
    'Tenant cashier',
    'cashier-read',
    'Read scoped cashier drawers, receipts and subscriber balances',
  );
  registerWorkspaceRead(
    app,
    options,
    {
      path: '/v1/tenants/:tenantId/operations/collections/workspace',
      operationId: 'readCollectionsWorkspace',
      permission: 'tenant.collection.view',
      action: 'tenant.collections.workspace.read',
      resourceType: 'collections_workspace',
      schema: z.object({}).strict(),
      querySchema: collectionsQuerySchema,
      execute: (w, id, v) => w.readCollectionsWorkspace(id, v as never),
    },
    'Tenant collections',
    'collections-read',
    'Read scoped routes, collectors, assignments and settlements',
  );
  registerWorkspaceRead(
    app,
    options,
